...
class BDD100K(LoadImagesAndLabels):  # for training
    """
    ...
    def __init__(self,
        ...
        if os.path.exists(lab_files) and not opt.val:
            ...
            with open(img_files) as json_file:
                self.img_files = json.load(json_file)
            with open(lab_files) as json_file:
                self.label_files = json.load(json_file)

        elif os.path.exists(lab_files) and opt.val:
            print("Loading existing validation img and label indexes...")
            with open(imgfilval) as json_file:
                self.img_files = json.load(json_file)
            with open(labfilval) as json_file:
                self.label_files = json.load(json_file)
        
        else:
            # ----- generate img and label file path lists
            print("Generating Img & Label Files...")
            ...
            if opt.val:
                print("Writing validation data index to JSON...")
                with open(imgfilval, 'w', encoding='utf-8') as f:
                    json.dump(self.img_files, f, ensure_ascii=False, indent=4)
                    
                with open(labfilval, 'w', encoding='utf-8') as f:
                    json.dump(self.label_files, f, ensure_ascii=False, indent=4)

            else:
                print("Writing data index to JSON...")
                with open(img_files, 'w', encoding='utf-8') as f:
                    json.dump(self.img_files, f, ensure_ascii=False, indent=4)
                    
                with open(lab_files, 'w', encoding='utf-8') as f:
                    json.dump(self.label_files, f, ensure_ascii=False, indent=4)

        if opt.id_weight > 0:  # If do ReID calculation
            
            if os.path.exists(tid_num) and not opt.val:
                print("Loading existing ID indexes...")
                ...
                if opt.val:
                    ...
                    print("Writing ID index to JSON...")
